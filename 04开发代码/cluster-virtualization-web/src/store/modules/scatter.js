        ...
        displayData(state) {
            ...
            if (data.length > 0 && state.samplingRate > 0) {
                ...
                for (let i = 0; i < dataset.length; ++i) {
                    let samplingData = [];
                    let totalDataNum = dataset[i].source.length;
                    let samplingNum = parseInt(totalDataNum * samplingRate);
                    let samplingDataIndexSet = new Set();
                    while (samplingDataIndexSet.size < samplingNum) {
                        let randomIndex = Math.floor(Math.random() * (totalDataNum));
